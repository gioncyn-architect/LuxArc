// ============================================================
// LuxArc AI — script.js COMPLETE v5 (BUGFIX)
...
// BugFix v4:
//   [1] AI Hair Color — kirim preset + palettes ke backend
//   [2] AI Makeup Blush — fix warna default + mapping category
//   [3] closeSubmenu flex-direction: column
//   [4] filterProducts display fix
//   [5] quick reply background color
// BugFix v5:
//   [6] AI Hair Color — hapus preset, pakai palettes saja
//       (preset YouCam pakai enum nama lengkap: "Lavender" bukan "purple")
//       Lebih aman pakai palettes hex langsung agar warna bebas
//   [7] Deteksi Otomatis — hapus fake scan hardcoded yang langsung
//       rekomendasikan kalung tanpa analisis nyata → ganti dengan
//       skin analysis sungguhan + rekomendasi produk berdasarkan hasil
// ============================================================
...
// ── Mapping nama warna → hex untuk Hair Color ────────────────
// [FIXED v5] Hapus preset string — YouCam preset pakai enum nama
// lengkap ("Lavender","Teal Blue", dll) bukan lowercase seperti "purple".
// Lebih aman & bebas warna pakai palettes hex langsung.
// Jika ingin preset resmi, gunakan nama PERSIS dari enum YouCam:
// "Jet Black","Chocolate Brown","Honey Blonde","Platinum Blonde",
// "Ash Gray","Rose Gold","Burgundy","Copper Red","Lavender","Teal Blue",
// "Plum Purple/Pastel Lilac","Midnight Blue/Denim Blue", dll.
...
// ── [FIXED v5] runAIHairColor ─────────────────────────────────
// Fix: hapus preset — YouCam preset enum tidak cocok dengan nama
//      warna Indonesia (misal "purple" → error, harus "Lavender")
// Fix: cukup kirim palettes dengan hex color saja — lebih aman & bebas
async function runAIHairColor(colorName, colorHex) {
    ...
    try {
        ...
        // Cek apakah colorHex valid, fallback ke mapping jika tidak
        const key = colorName.toLowerCase();
        const finalHex = (colorHex && colorHex.startsWith('#'))
            ? colorHex
            : (HAIR_COLOR_MAP[key] || '#7B2D8B');

        // [FIX] Tidak pakai preset — langsung kirim palettes saja
        const payload = {
            user_image_url: userImageUrl,
...
// ── Mapping kategori makeup → config YouCam API ──────────────
// pattern.name harus sesuai dengan label dari blush.json / eyeliner.json dll
const MAKEUP_CATEGORY_CONFIG = {
    blush: {
        category: 'blush',
        pattern: { name: '1color1' },   // valid label dari blush.json
        defaultColor: '#E8A0A0',        // warna rose/merah muda, bukan merah!
    },
    lip_color: {
        category: 'lip_color',
        shape: { name: 'original' },    // valid label dari lipshape.json
        style: { type: 'full' },
        defaultColor: '#CC3355',
    },
    eye_shadow: {
        category: 'eye_shadow',
        pattern: { name: '1color1' },   // valid label dari eyeshadow.json
        defaultColor: '#4B0082',
    },
    eye_liner: {
        category: 'eye_liner',
        pattern: { name: 'Arabic3' },   // valid label dari eyeliner.json
        defaultColor: '#000000',
    },
};

// ── Deteksi warna dari nama produk ───────────────────────────
...
// ── [FIXED v4] tryMakeupYoucam ────────────────────────────────
// Fix: warna default sebelumnya selalu #FF0000 (merah) untuk blush
//      → sekarang tiap kategori punya defaultColor sendiri
// Fix: zone → category agar backend bisa mapping ke YouCam dengan benar
function tryMakeupYoucam(makeupImgSrc, zone, makeupName, customColor = null) {
    // Normalisasi zone ke category YouCam yang valid
    let category = zone;
...
// ── [FIXED v4] runAIMakeup ────────────────────────────────────
// Fix: kirim "category" (bukan "zone") + pattern yang valid ke backend
//      Backend harus meneruskan field ini langsung ke YouCam API
//      tanpa ganti ke "lip_color" saat category adalah "blush"
async function runAIMakeup(category, color, makeupName) {
    ...
    try {
        ...
        // Bangun effect object sesuai spesifikasi YouCam API
        // Tiap kategori punya struktur yang berbeda
        let effectObject = { category };

        if (category === 'blush') {
            effectObject = {
                category: 'blush',
                pattern: config.pattern,       // { name: "1color1" }
                palettes: [{
                    color,
                    texture: 'matte',
                    colorIntensity: 65,
            ...
            effectObject = {
                category: 'lip_color',
                shape: config.shape,           // { name: "original" }
                style: config.style,           // { type: "full" }
                palettes: [{
                    color,
                    texture: 'matte',
                    colorIntensity: 80,
            ...
            effectObject = {
                category: 'eye_liner',
                pattern: config.pattern,       // { name: "Arabic3" }
                palettes: [{
                    color,
                    texture: 'matte',
                    colorIntensity: 90,
        ...
        const res = await fetch('/api/youcam?action=ai-makeup', {
            ...
            body: JSON.stringify({
                user_image_url: userImageUrl,
                // FIX: kirim "category" bukan "zone"
                category,
                color,
                makeup_name: makeupName,
                // FIX: kirim effect object lengkap agar backend bisa
                //      langsung forward ke YouCam tanpa perlu rebuild
                effect: effectObject,
            }),
        });
        const data = await res.json();
...
// Simpan riwayat chat untuk konteks Groq
let groqChatHistory = [];
...
// ── FIX: closeSubmenu — flex-direction: column ───────────────
...
// ── FIX: filterProducts — display 'flex' bukan 'block' ───────
...
// ── [FIXED v5] Deteksi Otomatis ──────────────────────────────
// Fix: sebelumnya hardcoded fake scan → langsung rekomendasikan
//      kalung tanpa analisis nyata (tidak masuk akal!)
// Fix: sekarang buka kamera → user ambil snapshot → upload ke
//      skin analysis API sungguhan → tampilkan hasil nyata
function triggerAutoDetect() {
    ...
    // Buka modal analisis kulit sungguhan
    setTimeout(() => {
...
async function runAutoDetectAnalysis() {
    ...
    try {
        ...
        // Hitung kondisi kulit dari skor
        ...
        // Tentukan rekomendasi produk berdasarkan hasil analisis nyata
        let skinType = '';
        let recommendations = [];
        ...
        const labels = {
            acne: '🔴 Jerawat', moisture: '💧 Kelembapan',
            pores: '⭕ Pori-pori', wrinkles: '〰️ Kerutan',
            radiance: '✨ Kecerahan', redness: '🔥 Kemerahan',
        };
        ...
        // Kirim ringkasan ke chat AI Advisor
