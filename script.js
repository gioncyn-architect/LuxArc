// ============================================================
// LuxArc AI — script.js v2025050104 (FIXED)
// ImgBB upload sebelum kirim ke Perfect Corp API
// ============================================================
...
async function runAIClothes(productImgSrc, productName) {
    ...
    try {
        ...
        // ✅ FIXED: Upload foto user ke ImgBB dulu → dapat URL publik
        ...
        // Ambil URL absolut gambar produk
        ...
        const res = await fetch('/api/youcam?action=ai-clothes', {
            ...
            body: JSON.stringify({
                user_image_url: userImageUrl,   // ✅ Kirim URL, bukan base64
        ...
        const outputUrl = data?.data?.results?.[0]?.url || data?.data?.result_url;
        if (outputUrl) {
...
async function runAIHairstyle() {
    ...
    try {
        ...
        const res = await fetch('/api/youcam?action=ai-hairstyle', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ user_image_url: userImageUrl, style }),  // ✅ Kirim URL
        });
        const data = await res.json();
        if (!res.ok) throw new Error(data.error || JSON.stringify(data));
...
async function runPhotoEnhancer() {
    ...
    try {
        ...
        const res = await fetch('/api/youcam?action=photo-enhance', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ user_image_url: userImageUrl }),  // ✅ Kirim URL
        });
        const data = await res.json();
        if (!res.ok) throw new Error(data.error || JSON.stringify(data));
