// ============================================================
// LuxArc AI — script.js
// FIX: 1) Harga NaN diperbaiki
//      2) YouCam API Key diambil dari Netlify Function (aman)
// ============================================================

// --- YouCam API Service ---
// API Key diambil secara aman dari Netlify Function (bukan hardcode)
class YouCamService {
    constructor() {
        this.apiKey    = null;
        this.apiSecret = null;
    ...
    // Panggil Netlify Function untuk ambil API key dari environment variable
    async init() {
        try {
            const res = await fetch('/.netlify/functions/get-youcam-key');
            if (!res.ok) throw new Error('Gagal mengambil API key dari server.');
            ...
            console.log('✅ YouCam API siap digunakan.');
            return true;
        } catch (err) {
            console.error('❌ YouCam API init gagal:', err.message);
            this.ready = false;
            return false;
        }
    }

    // Terapkan AR filter menggunakan API key yang sudah di-load
    applyFilter(stream, productId) {
        if (!this.ready) {
            console.warn('YouCam API belum siap. Pastikan Netlify Function aktif.');
            return;
        }
        // Titik integrasi SDK YouCam sesungguhnya
        // Gunakan this.apiKey dan this.apiSecret di sini saat SDK tersedia
        console.log(`✅ YouCam AR Filter aktif — Product: ${productId}`);
        console.log(`   Key: ${this.apiKey.slice(0,6)}*** | Secret: ${this.apiSecret.slice(0,4)}***`);
    }
}

// Inisiasi YouCam — langsung load key dari Netlify saat halaman terbuka
const youCamService = new YouCamService();
youCamService.init().then(ok => {
    if (!ok) {
        // Tampilkan peringatan halus, tidak crash
        console.warn('Fitur AR berjalan di mode simulasi karena API key tidak tersedia.');
    }
});
...
// --- FIX HARGA NaN ---
// Pastikan nilai selalu number valid sebelum diformat
function formatRupiah(number) {
    // Konversi ke number, hilangkan karakter non-numerik jika berupa string
    const num = typeof number === 'string'
        ? parseInt(number.replace(/\D/g, ''), 10)
        : Number(number);

    // Jika masih NaN setelah konversi, kembalikan '0'
...
async function openCamera(isAutoDetect = false) {
    ...
    try {
        ...
        // Gunakan YouCamService yang sudah load key dari Netlify
...
// FIX HARGA NaN: Pastikan price selalu number sebelum masuk cart
function addToCart(name, price) {
    // Konversi ke number, amankan dari nilai NaN
...
function openCheckout() {
    ...
    if (cart.length === 0) {
        ...
        // FIX HARGA NaN: Pastikan reduce tidak menghasilkan NaN
...
function openAdmin() {
    // FIX HARGA NaN: Amankan kalkulasi revenue di dashboard
