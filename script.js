// ============================================================
// LuxArc AI — script.js COMPLETE v10 (BUGFIX)
...
// BugFix v10:
//   [1] runAutoDetectAnalysis — fix error "undefined":
//       - Guard scores agar tidak crash jika response struktur berbeda
//       - Parse JSON dulu baru cek res.ok
//       - Tambah fallback || {} saat akses scores
//       - closeModal + switchPage dipanggil SETELAH result ditampilkan
// ============================================================
...
function handleQuickReply(type) {
    ...
    ['submenu-makeup','submenu-cat-rambut','submenu-skin'].forEach(id => {
...
// [FIXED v10] runAutoDetectAnalysis
// Fix: guard scores undefined, parse JSON dulu baru cek res.ok,
//      closeModal + switchPage dipanggil SETELAH result ditampilkan
async function runAutoDetectAnalysis() {
    ...
    try {
        ...
        // [FIX] Parse JSON dulu, baru cek ok
        let data;
        try {
            data = await res.json();
        } catch (jsonErr) {
        ...
        // [FIX] Guard scores — coba berbagai path, fallback {} agar tidak crash
        ...
        // [FIX] Tampilkan hasil dulu, baru close modal + switch page setelah delay
        document.getElementById('ai-result-area').innerHTML = resultHTML;
...
// ════════════════════════════════════════════════════════════
// Inject tombol Budget Filter ke category-scroll saat DOM siap
(function injectBudgetFilterUI() {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Tombol pemicu
        const budgetToggleBtn = document.createElement('button');
        budgetToggleBtn.className = 'cat-btn';
        budgetToggleBtn.id = 'btn-budget-filter';
        budgetToggleBtn.innerHTML = '💰 Budget';
        budgetToggleBtn.onclick = toggleBudgetPanel;
        catScroll.appendChild(budgetToggleBtn);

        // Panel slider budget
        const panel = document.createElement('div');
        panel.id = 'budget-filter-panel';
        panel.style.cssText = `
            display:none; padding:14px 16px; margin:0 12px 10px;
            background:rgba(26,26,26,0.95); border:1.5px solid rgba(255,215,0,0.3);
            border-radius:14px; animation: fadeInUp 0.25s ease;
        `;
        panel.innerHTML = `
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
                <span style="color:#FFD700;font-weight:600;font-size:0.9em;">💰 Filter Budget</span>
                <button onclick="resetBudgetFilter()" style="background:none;border:none;color:#888;font-size:0.78em;cursor:pointer;font-family:inherit;">Reset</button>
            </div>
            <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px;" id="budget-preset-btns">
                <button class="budget-preset-btn active" onclick="setBudgetPreset(this,0,9999999)" data-min="0" data-max="9999999">Semua</button>
                <button class="budget-preset-btn" onclick="setBudgetPreset(this,0,100000)" data-min="0" data-max="100000">≤ 100rb</button>
                <button class="budget-preset-btn" onclick="setBudgetPreset(this,100000,200000)" data-min="100000" data-max="200000">100–200rb</button>
                <button class="budget-preset-btn" onclick="setBudgetPreset(this,200000,500000)" data-min="200000" data-max="500000">200–500rb</button>
                <button class="budget-preset-btn" onclick="setBudgetPreset(this,500000,9999999)" data-min="500000" data-max="9999999">≥ 500rb</button>
            </div>
            <div style="display:flex;align-items:center;gap:10px;">
                <span style="color:#aaa;font-size:0.78em;white-space:nowrap;">Rp <span id="budget-min-label">0</span></span>
                <input type="range" id="budget-range-min" min="0" max="2500000" step="25000" value="0"
                    style="flex:1;accent-color:#FFD700;" oninput="onBudgetRangeChange()">
                <span style="color:#aaa;font-size:0.78em;">—</span>
                <input type="range" id="budget-range-max" min="0" max="2500000" step="25000" value="2500000"
                    style="flex:1;accent-color:#FFD700;" oninput="onBudgetRangeChange()">
                <span style="color:#aaa;font-size:0.78em;white-space:nowrap;">Rp <span id="budget-max-label">2.500.000</span></span>
            </div>
            <p id="budget-result-msg" style="color:#888;font-size:0.78em;margin-top:8px;text-align:center;"></p>`;
        catScroll.parentNode.insertBefore(panel, catScroll.nextSibling);
...
// ════════════════════════════════════════════════════════════
// Inject tombol Share & Diskon di nav-actions saat DOM siap
(function injectShareDiscountUI() {
    document.addEventListener('DOMContentLoaded', () => {
        // Tambah tombol Share ke nav
        ...
        // Inject kode diskon otomatis ke checkout modal: hook addToCart
        // (dilakukan via patchCartForDiscount di bawah)
    });
})();
...
// Patch openCheckout untuk tampilkan diskon otomatis
...
// Override openCheckout setelah DOM — dilakukan via DOMContentLoaded di bawah
document.addEventListener('DOMContentLoaded', () => {
    const _oc = window.openCheckout;
    window.openCheckout = function() {
        _oc && _oc();
        // Inject info diskon ke modal checkout
        setTimeout(() => {
...
// State before/after
let beforeAfterPairs = [];   // [{before: url, after: url, label: string}]

// Override showAIResult agar auto-save ke before/after jika ada foto user sebelumnya
...
window.showAIResult = function(containerId, outputUrl, filename) {
    ...
    // Coba ambil foto "before" dari input aktif
    const inputIds = ['user-photo-input','makeup-photo-input','hair-color-photo-input','skin-photo-input','acc-photo-input','enhance-photo-input','autodetect-photo-input'];
    let beforeUrl = null;
...
// Override openLookbook untuk sertakan tab Before/After
...
// Tambah tombol Before/After di showAIResult agar user bisa lihat langsung
const _origShowAIResult2 = window.showAIResult;
// Sudah di-override sekali, tidak perlu dobel — Before/After sudah masuk otomatis via override pertama.
