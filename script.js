// ============================================================
// LuxArc AI — script.js COMPLETE v11 (BUGFIX HAT & EARRING)
...
// BugFix v11:
//   [1] runAIHat — fix parameter mismatch:
//       - Ganti src_file_id → user_image_url
//       - Ganti ref_file_url → hat_image_url
//   [2] runAIEarring — fix parameter mismatch:
//       - Ganti src_file_url → user_image_url
//       - Ganti ref_file_urls + object_infos → earring_image_url
// ============================================================
...
async function runAIHat(hatImgSrc, hatName) {
    ...
    try {
        ...
        const res = await fetch('/api/youcam?action=ai-hat', {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                user_image_url: userImageUrl,  // ✅ FIXED: was src_file_id
                hat_image_url:  hatUrl,         // ✅ FIXED: was ref_file_url
                gender: 'female',
            }),
        });
        const data = await res.json();
...
// ════════════════════════════════════════════════════════════
// ── 9. AI EARRING — Virtual Try-On Anting (v11 FIXED) ───────
// ════════════════════════════════════════════════════════════
...
async function runAIEarring(earringImgSrc, earringName) {
    ...
    try {
        ...
        const res = await fetch('/api/youcam?action=ai-earring', {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                user_image_url:   userImageUrl,  // ✅ FIXED: was src_file_url
                earring_image_url: earringUrl,   // ✅ FIXED: was ref_file_urls + object_infos
            }),
        });
        const data = await res.json();
        if (!res.ok) throw new Error(data.error || JSON.stringify(data));
