// ============================================================
// LuxArc AI — script.js COMPLETE v4 (BUGFIX)
...
//       (sebelumnya hanya kirim color & color_name → null pattern)
//   [2] AI Makeup Blush — fix warna default (#FF0000 → rose)
//       + fix mapping zone → category agar tidak salah ke lip_color
//       + tambah pattern blush yang valid dari blush.json
...
// ── Mapping nama warna → hex preset YouCam ───────────────────
// Gunakan nama preset yang dikenal YouCam Hair Color API
const HAIR_COLOR_PRESETS = {
    'hitam':   { hex: '#1a1a1a', preset: 'black' },
    'coklat':  { hex: '#6B3A2A', preset: 'brown' },
    'merah':   { hex: '#CC0000', preset: 'red' },
    'oranye':  { hex: '#FF6600', preset: 'orange' },
    'kuning':  { hex: '#FFD700', preset: 'golden' },
    'hijau':   { hex: '#2E8B57', preset: 'green' },
    'biru':    { hex: '#1565C0', preset: 'blue' },
    'ungu':    { hex: '#7B2D8B', preset: 'purple' },
    'pink':    { hex: '#FF69B4', preset: 'pink' },
    'abu':     { hex: '#9E9E9E', preset: 'gray' },
    'silver':  { hex: '#C0C0C0', preset: 'silver' },
    'blonde':  { hex: '#F5DEB3', preset: 'blonde' },
    'platinum':{ hex: '#E8E8D0', preset: 'platinum' },
};
...
// ── [FIXED v4] runAIHairColor ─────────────────────────────────
// Fix: sebelumnya hanya kirim color & color_name → backend tidak punya
//      "preset" atau "palettes" yang dibutuhkan YouCam → null pattern error
// Fix: sekarang kirim preset (nama warna bahasa Inggris) + palettes sebagai fallback
async function runAIHairColor(colorName, colorHex) {
    ...
    try {
        ...
        // Cari preset YouCam berdasarkan nama warna (case-insensitive)
        const key = colorName.toLowerCase();
        const presetData = HAIR_COLOR_PRESETS[key] || null;
        const presetName = presetData?.preset || null;

        // Bangun payload — jika ada preset pakai preset (prioritas),
        // selalu sertakan palettes sebagai fallback
        const payload = {
            user_image_url: userImageUrl,
            color_name: colorName,
            // FIX: tambahkan preset jika ditemukan
            ...(presetName ? { preset: presetName } : {}),
            // FIX: selalu kirim palettes agar backend punya fallback
            palettes: [
                {
                    color: colorHex,
                    colorIntensity: 80,
