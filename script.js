  ...
  try {
    ...
    if (action === 'ai-makeup') {
      ...
      if (frontendEffect && frontendEffect.category) {
        ...
        if (cat === 'blush') {
          effectObject = { category: 'blush', pattern: { name: '1color1' }, palettes: [{ color: col, texture: 'matte', colorIntensity: 65 }] };
        } else if (cat === 'eye_shadow' || cat === 'eye' || cat === 'eyes') {
          effectObject = { category: 'eye_shadow', pattern: { name: '1color1' }, palettes: [{ color: col, texture: 'shimmer', shimmerColor: col, shimmerIntensity: 50, colorIntensity: 50 }] };
        } else if (cat === 'eye_liner' || cat === 'liner') {
          effectObject = { category: 'eye_liner', pattern: { name: 'Arabic3' }, palettes: [{ color: col, texture: 'matte', colorIntensity: 90 }] };
        } else if (cat === 'bronzer') {
          effectObject = { category: 'bronzer', pattern: { name: 'Bronzer1' }, palettes: [{ color: col, colorIntensity: 50 }] };
        } else {
          effectObject = { category: 'lip_color', shape: { name: 'original' }, style: { type: 'full' }, morphology: { fullness: 0, wrinkless: 0 }, palettes: [{ color: col, texture: 'matte', colorIntensity: 80 }] };
        }
      }
    ...
    // ── 5. AI Hair Color ────────────────────────────────────
    // [FIX v11 - VALID] Payload resmi: { src_file_url, preset: "NamaPreset" }
    // Sumber: YouCam API Playground resmi (yce.makeupar.com)
    if (action === 'ai-hair-color') {
      ...
      if (!resolvedPreset) {
        return res.status(400).json({
          ...
          available_presets: [...new Set(Object.values(HAIR_PRESET_MAP))],
        });
      }
    ...
    if (action === 'ai-hairstyle') {
      ...
      const out = await runTask('/s2s/v2.1/task/hair-transfer', {
        src_file_url: user_image_url,
    ...
    return res.status(400).json({
      ...
      available_actions: ['skin-analysis', 'ai-clothes', 'ai-necklace', 'ai-makeup', 'ai-hair-color', 'ai-hairstyle', 'ai-look'],
    });
