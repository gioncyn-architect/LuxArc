...
// ════════════════════════════════════════════════════════════
// Budget filter tombol dihapus dari category-scroll per rencana revisi.
// Semua logika filter tetap tersedia jika dipanggil secara programatik.
(function injectBudgetFilterUI() {
    document.addEventListener('DOMContentLoaded', () => {
        // Tombol Budget tidak lagi diinjek ke category-scroll.
        // Panel budget filter dipertahankan namun tidak otomatis ditampilkan.
...
function renderCartPage() {
    ...
    // Update voucher chips highlight
...
function filterProducts(category, btn) {
    ...
    document.querySelectorAll('.product-card').forEach(card => {
        ...
        if (category === 'semua') {
            ...
            // pakaian mencakup: pakaian, atasan, dress
...
function addToCart(name, price) {
    ...
    // refresh cart page if currently open
...
async function runAIHat(hatImgSrc, hatName) {
    ...
    try {
        const userBase64 = await fileToBase64(userInput.files[0]);
        const userFileId = await uploadToImgBB(userBase64); // returns URL, used as src_file_id param
...
// ════════════════════════════════════════════════════════════
// ── AI EARRING — Virtual Try-On Anting ──────────────────────
// ════════════════════════════════════════════════════════════
...
async function runAIEarring(earringImgSrc, earringName) {
    ...
    try {
        ...
        const earringUrl = earringImgSrc.startsWith('http')
            ? earringImgSrc
            : window.location.origin + '/' + earringImgSrc.replace(/^\//, '');
        const res = await fetch('/api/youcam?action=ai-earring', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                src_file_url: userImageUrl,
                ref_file_urls: [earringUrl],
                object_infos: [{
                    object_type: 'earring',
                    earring_type: 'drop',
                    position: 'both',
