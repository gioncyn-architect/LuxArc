...
export default async function handler(req, res) {
  ...
  // ── Ekstrak URL hasil dari berbagai format response ──────────
  ...
  async function pollTask(taskId, endpoint, maxRetry = 30, intervalMs = 2000) {
    for (let i = 0; i < maxRetry; i++) {
      ...
      if (['success', 'completed', 'done', 'finish', 'finished'].includes(status)) {
        return d;
      }
  ...
  async function runTask(endpoint, payload) {
    ...
    let result;
    try {
      result = await pollTask(taskId, endpoint);
    } catch (pollErr) {
      return { error: pollErr.message, status: 500 };
    }

    const resultUrl = extractResultUrl(result);
    if (!resultUrl) {
      return {
        error: 'AI selesai tapi tidak ada URL hasil. Response: ' + JSON.stringify(result).slice(0, 300),
        detail: result,
        status: 500,
      };
    }
